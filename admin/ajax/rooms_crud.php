...
if (isset($_POST['add_room'])) {
    ...
    //q2 is execute lots of tym which take time that's why ($stmt = mysqli_prepare($con,$q2) code run to save time......
    ...
    //q3 is execute lots of tym which take time that's why ($stmt = mysqli_prepare($con,$q2) code run to save time......
...
// Showing rooms Details..
if (isset($_POST['get_room'])) {
    $frm_data = filteration($_POST);

    $res1 = select("SELECT * FROM `rooms` WHERE `id`=?", [$frm_data['get_room']], 'i');
    $res2 = select("SELECT * FROM `room_features` WHERE `room_id`=?", [$frm_data['get_room']], 'i');
    // $res2 = select("SELECT * FROM `room_features` WHERE 1,[$frm_data['get_room']],'i');
...
if (isset($_POST['get_room_images'])) {
    ...
    while ($row = mysqli_fetch_assoc($res)) {
        if ($row['thumb'] == 1) {
            ...
            $thumb_btn = "<button type='button' onclick='thumb_image($row[s_no],$row[room_id])' class='btn btn-secondary shadow-none'>
        ...
        echo <<<data
            <tr class="align-middle">
                ...
                <td>$thumb_btn</td>
                <td>
                    <button type='button' onclick='rem_image($row[s_no],$row[room_id])' class='btn btn-danger shadow-none'>
                        <i class='bi bi-trash'></i>
                    </button>
                </td>
            </tr>
...
//deleting images form room....
if (isset($_POST['rem_image'])) {
    $frm_data = filteration($_POST);
    $values = [$frm_data['image'], $frm_data['room_id']];

    $pre_q = "SELECT * FROM `room_images` WHERE `s_no`=? AND `room_id`=?";
    $res = select($pre_q, $values, 'ii');
    $img = mysqli_fetch_assoc($res);

    if (deleteImage($img['image'], ROOMS_FOLDER)) {
        $q = "DELETE FROM `room_images` WHERE `s_no`=? AND `room_id`=?";
...
// Selecting Images b/w 2or more images in room....
if (isset($_POST['thumb_image'])) {
    ...
    $q = "UPDATE `room_images` SET `thumb`=? WHERE `s_no`=? AND `room_id`=?";
